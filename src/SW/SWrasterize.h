...
sw_inline void _swProcessTriangle_correct(SWprogram *p, SWframebuffer *f, SWfloat vs_out[3][SW_MAX_VTX_ATTRIBS], SWint _0, SWint _1,
	...
	if (min[0] < 0) min[0] = 0;				if (min[1] < 0) min[1] = 0;
	if (max[0] >= f->w) max[0] = f->w - 1;  if (max[1] >= f->h) max[1] = f->h - 1;

	min[0] &= ~(SW_TILE_SIZE - 1);   min[1] &= ~(SW_TILE_SIZE - 1);

    d01[0] = p0[0] - p1[0]; d01[1] = p0[1] - p1[1];
    d12[0] = p1[0] - p2[0]; d12[1] = p1[1] - p2[1];
    d20[0] = p2[0] - p0[0]; d20[1] = p2[1] - p0[1];
			...
			if (a == 15 && b == 15 && c == 15 && full_tile) {
				...
				for (iy = y; iy < y + SW_TILE_SIZE; iy++) {
					...
					for (ix = x; ix < x + SW_TILE_SIZE; ix++) {
                        ...
                        uvw[0] *= inv_w; uvw[1] *= inv_w; uvw[2] *= inv_w;
